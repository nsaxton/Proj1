...
void CFish::Update(double elapsed)
{
    ...
    if(mSpeedX < 0 && GetX() <= 50+GetWidth()/2+GetXOffset())
    {
        mSpeedX = -mSpeedX;
    }
    
    if(mSpeedY > 0 && GetY() >= (GetAquarium()->GetHeight()-100-GetHeight()/2)+GetYOffset())
    {
        mSpeedY = -mSpeedY;
    }
    if(mSpeedY < 0 && GetY() <= 50+GetHeight()/2+GetYOffset())
    {
        mSpeedY = -mSpeedY;
    }
